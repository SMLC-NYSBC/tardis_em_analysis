...
def curvature(coord: np.ndarray, tortuosity_b=False, s=1) -> Union[float, tuple]:
    """
...
def intensity_list(
    ...
    if normalize_image:
        max_, min_ = np.max(image), np.min(image)
...
def calculate_spline_correlations(
    ...
    max_, min_ = np.max(image_stack[frame_id]), np.min(image_stack[frame_id])
    frame_ref = (image_stack[frame_id] - min_) / (max_ - min_)
    ...
    for spline_id in num_splines:
        ...
        correlations_px[int(spline_id)]["reference"] = [
            float(f) if is_floatable(f) else 0.0 for f in ref_intensity
        ]
        correlations_px[int(spline_id)]["MT"] = {}
        for i in range(N):
            max_, min_ = np.max(image_stack[i]), np.min(image_stack[i])
            spline_intensity = (
                pixel_intensity(pixel_coords, ((image_stack[i] - min_) / (max_ - min_)))
                - spline_background
            )
